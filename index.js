...
(function () {
  ...
  const animateCount = (el) => {
    ...
    const duration = prefersReduced ? 0 : 1600; // calm
  ...
  if (checkBtn && statusDiv && kodeInput) {
    checkBtn.addEventListener('click', function () {
      const kode = kodeInput.value.trim();
      statusDiv.innerHTML = '';
      if (kode === '') { statusDiv.innerHTML = '<span style="color: #a94442;">Silakan masukkan kode laporan terlebih dahulu.</span>'; return; }
      checkBtn.disabled = true;
      const spinner = document.createElement('span'); spinner.className = 'spinner'; spinner.setAttribute('aria-hidden', 'true');
      const loadingText = document.createElement('span'); loadingText.style.marginLeft = '8px'; loadingText.textContent = 'Memeriksa...';
      statusDiv.appendChild(spinner); statusDiv.appendChild(loadingText);
      setTimeout(() => {
        let msg = '';
        if (kode === 'PPKS123') msg = '<span style="color: #2e7d32;">Status: Laporan Anda sedang dalam tahap Verifikasi Awal oleh tim kami.</span>';
        else if (kode === 'PPKS456') msg = '<span style="color: #1565c0;">Status: Laporan telah dilanjutkan ke tahap Investigasi.</span>';
        else msg = '<span style="color: #ef6c00;">Kode laporan tidak ditemukan. Pastikan kode benar, ya.</span>';
  ...
  if (boxBlue && !prefersReduced) {
    const onScroll = () => { boxBlue.style.transform = `translateY(${window.scrollY * 0.06}px)`; };
